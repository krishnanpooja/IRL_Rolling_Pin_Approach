...
def speed(min_max_scaler):
    df = pd.read_csv(r'C:\Users\ga67zod\Desktop\Final_IRL\All files_feature_classification\whole_dataset_new1.csv',delimiter ="\t").fillna(value = 0)
    df = df[df['ID'] < 45028]
    df_x = pd.read_csv(r'C:\Users\ga67zod\Desktop\Final_IRL\x_new_whole.txt',delimiter ="\t").fillna(value = 0)
    df['x']=df_x
    df_y = pd.read_csv(r'C:\Users\ga67zod\Desktop\Final_IRL\y_new_whole.txt',delimiter ="\t").fillna(value = 0)
    df['y']=df_y 
    df_new_speed = pd.read_csv(r'C:\Users\ga67zod\Desktop\Final_IRL\new_speed_trial.txt',delimiter ="\t").fillna(value = 0)
    df['speed']=df_new_speed 
    ...
    check=False
    df_sub = df.loc[( (df['ID']<=900)),['Lane','Delta_v_F','Dist_F','Delta_v_F2','Dist_F2','Delta_v_FL','Dist_FL','Delta_v_FR','Dist_FR']]
    print(df_sub.shape)
    for v_id in vehicle_id:
        ...
        if check:
            ...
            speed=np.append(speed,new[:, [6]],axis=0)
        else:
            X=new
            speed=new[:, [6]]
            check=True
...
def main(grid_size_x,grid_size_y,discount, n_trajectories, epochs, learning_rate):
    ...
    vehicle_id = vehicle_id[:int(len(vehicle_id) *0.95)]
